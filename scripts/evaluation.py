...
def run_complete_test(executor_name, test_case, model, scorer):
    """Run all questions over a specific document."""
    chat_with_data = HistoryBasedChat.create(executor_name, document=test_case, model=model)
    test_results = {
...
def eval_conv_fin_qa(executor_name, model, file_name=None, sample_size=None, parallel_exec=True):
    ...
    with open(f'results_{datetime.today().strftime("%Y_%m_%d-%H_%M_%S")}', 'w', encoding='utf-8') as f:
